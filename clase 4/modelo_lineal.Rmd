...
```{r}
plot(mammals$BrainWt, mammals$BodyWt)
plot(mammals$BrainWt, mammals$BodyWt, xlim = c(0,1000), ylim = c(0,1000))
```
...
plot(bdims$wgt, bdims$hgt, col= colores)
```
...
```{r}
plot(smoking$age,smoking$amtWeekends)
```
...
```{r}
mammals$Species[mammals$BrainWt>4000]
mammals2 <- mammals[mammals$BrainWt<4000,]
plot(mammals2$BrainWt, mammals2$BodyWt)  #Sigue teniendo un outlier y relación no lineal

cor(mammals2$BrainWt, mammals2$BodyWt)
...
```{r}
mammals$logBrainWt <- log(mammals$BrainWt)
mammals$logBodyWt <- log(mammals$BodyWt)

plot(mammals$logBrainWt, mammals$logBodyWt)
cor(mammals$logBrainWt, mammals$logBodyWt)
```
...
cor(mammals$logBrainWt, mammals$logBodyWt, method = "spearman")
```
