...
Realizamos los graficos para el modelo completo y uno de los modelos con mayor deviance (student+income).
...
Hosmer_Lemeshow_plot <- function(dataset, pred_column, group_column, bins, positive_value){
  
  dataset['group'] <- bin(dataset[pred_column], nbins = bins, method = 'l', labels=c(1:bins))
  
  positive_class <- dataset %>% filter(!!sym(group_column)==positive_value) %>% group_by(group) %>% count()
  HL_df <- dataset %>% group_by(group) %>% summarise(pred=mean(!!sym(pred_column)), count=n()) %>%
            inner_join(.,positive_class) %>%
            mutate(freq=n/count)
  
  HM_plot <- ggplot(HL_df, aes(x=pred, y=freq)) + geom_point(aes(size=n), color='forestgreen') +
...
Hosmer_Lemeshow_plot(default, 'predicted_full', 'default', 10, 'Yes')
