...
models <- data_frame(logit_formulas) %>% # dataframe a partir del objeto formulas
  mutate(models = names(logit_formulas), # columna con los nombres de las formulas
         ...
         mod = map(logit_formulas, ~  glm(.,family = 'binomial', data = default))) # Que estamos haciendo acá? Que vamos a estar en la columna?
```
...
models %>% 
  ...
  select(-c(null.deviance, df.null, AIC, BIC)) %>% 
  arrange(deviance)
```
...
violin_full=ggplot(prediction_full, aes(x=default, y=.fitted)) + 
  geom_violin(aes(fill=default), fill = "forestgreen") +
  theme_bw() +
  guides(fill=FALSE) +
...
violin_bad=ggplot(prediction_bad, aes(x=default, y=.fitted)) + 
  geom_violin(aes(fill=default), fill="firebrick") + 
  theme_bw() +
  guides(fill=FALSE) +
...
ggplot(logit_pred, aes(cutoff,estimate, group=term, color=term)) + geom_line(size=1) + theme_bw() + labs(title= 'Accuracy, Sensitivity y Specificity', subtitle= 'Modelo completo')
```
...
¿Cómo se relaciona este gráfico con el de la curva ROC?
