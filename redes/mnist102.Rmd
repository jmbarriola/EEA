---
title: "Clase x. Redes Neuronales II^[Estas notas estan basadas en https://tensorflow.rstudio.com/keras/#tutorials]"
...
saveRDS(fit_history,"results/cnn_hist.RDS")
```

```{r include=FALSE, eval=T}
#levanto la historia. No lo muestro, pero lo corro por default
fit_history <- read_rds("results/cnn_hist.RDS")
```
...
```{r eval=FALSE, echo=T}
model %>% save_model_hdf5("results/cnn_model.h5")
```

y para cargarlo

```{r}
modelo_preentrenado <- load_model_hdf5("results/cnn_model.h5")
```
